...
import { Loader2, CheckSquare, Camera, AlertCircle } from "lucide-react";
...
export const DeliveryChecklistForm = () => {
  ...
  const [kmsRegistro, setKmsRegistro] = useState<number>(0);
  ...
  useEffect(() => {
    if (selectedVehicle) {
      ...
      if (vehicle) {
        setKmsRegistro(vehicle.kilometraje_actual);
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await logAudit({
        ...
        description: "Checklist de entrega completado"
      });

      toast.success("Checklist de entrega guardado exitosamente");
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Completa el checklist de entrega del vehículo al cliente
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <Alert>
          ...
          <AlertDescription>
            Este formulario debe ser completado por personal comercial u operativo al momento de entregar un vehículo.
          </AlertDescription>
        </Alert>
        ...
        {selectedVehicle && (
          <div className="space-y-2">
            <Label htmlFor="kms">Kilometraje al Momento de Entrega</Label>
            <Input
              id="kms"
              type="number"
              value={kmsRegistro}
              onChange={(e) => setKmsRegistro(Number(e.target.value))}
            />
          </div>
        )}
        ...
        <Button 
          ...
          disabled={!selectedTemplate || !selectedVehicle || loading}
          className="w-full"
        >
          {loading ? (
            <>
