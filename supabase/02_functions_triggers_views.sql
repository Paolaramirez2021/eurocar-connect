...
-- Description: Functions, Triggers and Views
...
-- Function: handle_new_user (creates profile on signup)
...
-- Function: has_role (check if user has specific role)
...
-- Function: has_any_role (check if user has any of the specified roles)
...
-- Function: set_contract_number (trigger function)
...
-- Function: check_maintenance_alerts
...
-- Function: mark_maintenance_alert_resolved
...
-- Function: create_finance_item_for_maintenance
...
-- Function: create_finance_item_for_reservation
...
-- Function: prevent_contract_modification
...
-- Function: update_reservation_on_contract_create
...
BEGIN
    IF NEW.reservation_id IS NOT NULL THEN
        ...
        WHERE id = NEW.reservation_id AND estado = 'pending';
    END IF;
    RETURN NEW;
END;
$$;

-- Function: update_reservation_on_contract_delete
...
BEGIN
    IF OLD.reservation_id IS NOT NULL THEN
        UPDATE public.reservations
        SET estado = 'pending', updated_at = now()
        WHERE id = OLD.reservation_id AND estado = 'confirmed';
    END IF;
    RETURN OLD;
END;
$$;

-- Function: update_vehicle_estado_on_reservation
...
BEGIN
    IF NEW.estado IN ('confirmed', 'pending') THEN
        ...
        SET estado = 'alquilado', updated_at = now()
...
-- Function: update_vehicle_estado_on_reservation_end
...
-- Function: update_vehicle_estado_on_maintenance
...
BEGIN
    UPDATE public.vehicles
    SET estado = 'mantenimiento', updated_at = now()
    WHERE id = NEW.vehicle_id;
    RETURN NEW;
END;
$$;

-- Function: update_vehicle_on_maintenance_complete
...
BEGIN
    IF NEW.kms IS NOT NULL THEN
        UPDATE public.vehicles
        SET kilometraje_actual = NEW.kms, updated_at = now()
        WHERE id = NEW.vehicle_id;
    END IF;
    
    IF NEW.tipo ILIKE '%soat%' THEN
        UPDATE public.vehicles
        SET fecha_soat = NEW.fecha + INTERVAL '1 year', updated_at = now()
        WHERE id = NEW.vehicle_id;
    END IF;
    
    IF NEW.tipo ILIKE '%tecnomecánica%' OR NEW.tipo ILIKE '%tecnomecanica%' THEN
        UPDATE public.vehicles
        SET fecha_tecnomecanica = NEW.fecha + INTERVAL '1 year', updated_at = now()
        WHERE id = NEW.vehicle_id;
    END IF;
    
    IF NEW.tipo ILIKE '%impuesto%' THEN
        UPDATE public.vehicles
        SET fecha_impuestos = NEW.fecha + INTERVAL '1 year', updated_at = now()
        WHERE id = NEW.vehicle_id;
    END IF;
...
-- Function: notify_reservation_expired
...
-- ============================================================
-- END OF FILE 02_functions_triggers_views.sql
-- ============================================================
